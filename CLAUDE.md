...
7.1. **Бэкап БД** ← *сейчас здесь* — `pg_dump` по cron на сервере, ротация 7 дней,
   копия в Hetzner Object Storage (S3-совместимое). Защита от потери данных при
   неудачной миграции или случайном удалении.
...
20. **Локализация API / Swagger** — публичный `/docs` открыт рекрутерам, поэтому внешние тексты
